...
\paragraph{Symmetry-derived MDS.}
The Harmony Worldwide resonance matrix exhibits a 19/40 square symmetry
split among Other Metal elements~\cite{HW26}. Three square groups
(a, b, c) each contain 4 columns, with groups a and b showing identical
symmetry counts across all five symmetry types. This structural regularity
mirrors the properties required of an MDS matrix. Can the 19/40 split
inform MDS matrix design directly, replacing circulant construction with
one derivable from the same harmonic framework as the round constants?
...
The flat prover time profile is the most practically significant result: it
demonstrates that the variable-width design can be used freely in ZK
applications without choosing a fixed state width at design time. The
construction self-selects the optimal width for the input, and the prover
pays essentially the same cost regardless.
...
Circom circuits for all four widths, gnark Groth16 circuits, and a code
generator supporting any width and field.
